...
const { query, sqlDate } = require('../../../database/db');
const {
  addDaysToUser,
  removeDaysFromUser,
  toggleUserAccount
...
async function getUsers(req, res) {
  try {
    ...
    if (search) {
      sql += ` AND (LOWER(username) LIKE LOWER($${paramIndex}) OR LOWER(email) LIKE LOWER($${paramIndex}))`;
      params.push(`%${search}%`);
      paramIndex++;
    }
  ...
  try {
    ...
    const usersResult = await query(`
      SELECT 
        ...
        SUM(CASE WHEN created_at > ${sqlDate('-7 days')} THEN 1 ELSE 0 END) as new_last_week
      FROM users
    `);
    ...
    const paymentsResult = await query(`
      SELECT 
        ...
        COALESCE(SUM(CASE WHEN created_at > ${sqlDate('-30 days')} THEN amount ELSE 0 END), 0) as revenue_last_month
      FROM payments
    ...
    const auctionsResult = await query(`
      SELECT 
        COUNT(*) as total,
        SUM(CASE WHEN started_at > ${sqlDate('-7 days')} THEN 1 ELSE 0 END) as last_week,
        SUM(CASE WHEN DATE(started_at) = DATE(${sqlDate()}) THEN 1 ELSE 0 END) as today
      FROM auctions
    `);
...
async function getAllPayments(req, res) {
  try {
    ...
    const offset = (page - 1) * limit;
    let sql = `SELECT p.*, u.username, u.email FROM payments p LEFT JOIN users u ON u.id = p.user_id WHERE 1=1`;
    const params = [];
    let idx = 1;
    ...
    res.json({ payments: r.rows || r, total: (r.rows || r).length });
  } catch (error) {
...
async function refundPayment(req, res) {
  try {
    ...
    if (!r.rows || r.rows.length === 0) return res.status(404).json({ error: 'Pago no encontrado' });
    const payment = r.rows[0];
    /* also adjust user plan: remove the days that were added */
    if (payment.days_added > 0) {
      await query(
        `UPDATE users SET plan_days_remaining = MAX(0, plan_days_remaining - $1) WHERE id = $2`,
        [payment.days_added, payment.user_id]
      );
      await query(
        `INSERT INTO plan_history (user_id, action, plan_type, days_changed, admin_id, notes) VALUES ($1, 'refund', 'free', $2, $3, $4)`,
        [payment.user_id, -payment.days_added, req.user.userId, `Refund de pago #${id}`]
      );
    }
    res.json({ success: true, message: 'Pago reembolsado', payment: r.rows[0] });
  } catch (error) {
