  ...
  try {
    ...
    await query(
      `INSERT INTO sessions (user_id, refresh_token_hash, ip_address, expires_at)
       VALUES ($1, $2, $3, $4)`,
      [user.id, refreshHash, req.ip || 'unknown', expiresAt.toISOString()]
    );
...
async function refreshToken(req, res) {
  try {
    ...
    const sessionInfo = await query(
      `SELECT s.*, u.id as user_id, u.username, u.email, u.role, u.tiktok_target_idc FROM sessions s JOIN users u ON s.user_id = u.id WHERE s.refresh_token_hash = $1 AND s.expires_at > ${sqlDate()}`,
    ...
    try { await query('UPDATE sessions SET last_used_at = CURRENT_TIMESTAMP WHERE id = $1', [sessionInfo.rows[0].id]); } catch {} // columna opcional
