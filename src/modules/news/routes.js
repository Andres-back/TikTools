  ...
  try {
    ...
    const processedNews = news.map(item => {
      if (item.image_url) {
        ...
        const fullPath = path.join(process.cwd(), item.image_url);
...
router.delete('/:id', authenticateToken, isAdmin, async (req, res) => {
  try {
    ...
    if (news && news.image_url) {
      const imagePath = path.join(process.cwd(), news.image_url);
...
router.post('/cleanup', authenticateToken, isAdmin, async (req, res) => {
  try {
    ...
    for (const news of newsWithImages) {
      if (news.image_url) {
        const fullPath = path.join(process.cwd(), news.image_url);
