...
async function recordGift(req, res) {
  try {
    ...
    // Buscar o crear donador (con reintento si hay race condition)
    let donorId;
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      if (donorResult.rows.length === 0) {
        try {
          ...
          if (attempt < 2 && e.message?.includes('UNIQUE') || e.code === '23505') continue;
          throw e;
        }
      } else {
    ...
    if (isNewDonor) {
      await query(`UPDATE auctions SET unique_donors = unique_donors + 1 WHERE id = $1`, [id]);
    }
...
async function getStats(req, res) {
  try {
    ...
    const recentResult = await query(
      `SELECT 
         COUNT(CASE WHEN started_at > ${sqlDate('-7 days')} THEN 1 END) as auctions_last_week,
         SUM(CASE WHEN started_at > ${sqlDate('-7 days')} THEN total_coins_collected ELSE 0 END) as coins_last_week,
         COUNT(CASE WHEN started_at > ${sqlDate('-30 days')} THEN 1 END) as auctions_last_month
       FROM auctions 
       WHERE user_id = $1`,
      [req.user.userId]
    );
